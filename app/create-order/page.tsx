...
export default function CreateOrder() {
  ...
  const handleServiceSelect = (serviceId: number, serviceName: string, price: number) => {
    ...
    const defaultTechPassport = needsTechPassport
      ? {
          ...
          processingType: {
            manual: false,
            bioTechnology: true, // Автоматически включаем биотехнологию
          },
    ...
    const newService: OrderService = {
      ...
      techPassport: defaultTechPassport, // Добавляем технический паспорт если нужен
    }
  ...
  const handleTechPassportToggle = (serviceId: number, value: boolean) => {
    setOrderServices(
      orderServices.map((service) => {
        if (service.id === serviceId) {
          // Создаем пустой технический паспорт с дефолтными значениями
  ...
  const validateField = (name: string, value: any): string => {
    switch (name) {
      ...
      case "phone":
        ...
        return /^\+?[0-9\s()-]{10,18}$/.test(value) ? "" : "Неверный формат телефона"
      case "email":
        if (!value.trim()) return ""
            ...
            <div className="col-span-6 relative">
              <input
                ...
                className={`w-full px-3 py-2 border ${
                  touched.companyName && errors.companyName ? "border-[#e74c3c]" : "border-gray-300"
            ...
            <>
              <div className="col-span-3 relative">
                <input
                  ...
                  className={`w-full px-3 py-2 border ${
                    touched.surname && errors.surname ? "border-[#e74c3c]" : "border-gray-300"
                  } rounded-full text-sm`}
                />
        ...
        <div className="grid grid-cols-12 gap-4 mt-4">
          ...
          <div className="col-span-3 relative">
            <input
              ...
              className={`w-full px-3 py-2 border ${
                touched.phone && errors.phone ? "border-[#e74c3c]" : "border-gray-300"
              } rounded-full text-sm`}
            />
          ...
          <div className="col-span-3 relative">
            <input
              ...
              className={`w-full px-3 py-2 border ${
                touched.email && errors.email ? "border-[#e74c3c]" : "border-gray-300"
              } rounded-full text-sm`}
            />
      ...
      <div className="bg-[#f7f7f7] rounded-lg p-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            {/* Промокод и действующие акции */}
            <tr className="border-t border-gray-200">
              <td colSpan={2} className="py-3 px-4 text-sm font-medium">
                Итого:
              </td>
              <td colSpan={2} className="py-3 px-4 text-sm text-gray-500">
                <div className="relative" ref={promocodeDropdownRef}>
                  ...
                  {showPromocodeDropdown && (
                    <div className="absolute z-50 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      {getActivePromocodes()
                        .filter((promocode) => {
                          // Фильтруем промокоды по целевой аудитории
                          return (
              ...
              <td className="py-3 px-4 text-sm">
                <div className="flex flex-col">
                  ...
                  <span className={`font-medium ${totalDiscount > 0 || promocodeDiscount > 0 ? "text-green-600" : ""}`}>
                    {finalAmount} ₽
                  </span>
                </div>
              </td>
