...
import { Trophy, ThumbsUp, TrendingUp, AlertTriangle, XCircle, Settings, RefreshCw } from "lucide-react"
...
export default function RFMAnalysis() {
  ...
  const calculateRFMSegments = (): RFMSegment[] => {
    ...
    const clientStats = clients.map((client) => {
      ...
      const monetary = clientOrders.reduce((sum, order) => sum + order.discountedAmount, 0)
    ...
    const champions = clientStats.filter(
      (stat) => stat.recency <= 30 && stat.frequency >= 5 && stat.monetary >= settings.monetaryAmount,
    )

    const loyalCustomers = clientStats.filter(
      (stat) => stat.recency <= 90 && stat.frequency >= 3 && stat.monetary >= 2000 && !champions.includes(stat),
    )
    ...
    return [
      {
        ...
        criteria: `R: 1-30 дн. | F: 5+ раз | M: >${settings.monetaryAmount} руб.`,
      ...
      },
      {
        ...
        criteria: `R: 31-90 дн. | F: 3-5 раз | M: 2000-5000 руб.`,
      ...
      },
      {
        ...
        criteria: `R: 1-60 дн. | F: 1-2 раз | M: 1000-3000 руб.`,
      ...
      },
      {
        ...
        criteria: `R: 91-180 дн. | F: 1-3 раз | M: <2000 руб.`,
      ...
      },
      {
        ...
        criteria: `R: >180 дн. | F: 1-2 раз | M: <1000 руб.`,
        aiRecommendation:
